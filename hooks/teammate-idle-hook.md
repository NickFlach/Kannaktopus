...
This hook enables event-driven agent scheduling for Kannaktopus workflows.

## Purpose

When a teammate agent finishes its current work and enters an idle state, this hook:

1. Checks for an active kannaktopus workflow phase
...
- `~/.kannaktopus/session.json` - Workflow session state with agent queue
